package com.dotop.smartwater.project.third.concentrator.filter;
...
public class AuthFilter implements Filter, UserStore {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        try {
            ...
            if (requestURI.contains("/test")) {
            ...
            } else if (requestURI.contains("/login_enterprise")) {
                chain.doFilter(request, response);
                ...
                chain.doFilter(request, response);
            } else if (requestURI.contains("/third/device")) {
                chain.doFilter(request, response);
            }
            //socket调试
            else if (requestURI.contains("/api") || requestURI.contains("/back")) {
                chain.doFilter(request, response);
            } else {
