 ...
 *
 */
public class CombineMsgHandler23 extends MsgHandler {

    private static final Logger logger = LogManager.getLogger(CombineMsgHandler23.class);
...
//        water = new BigDecimal(water).divide(new BigDecimal(100)).toString();
        ...
        // CELID
        String celidHex = contentHex.substring(114, 118);
        String celid = NumberUtils.hexToTenStr(celidHex);
        // RSRP
        String rsrpHex = contentHex.substring(118, 122);
        String rsrp = NumberUtils.hexToTenStr(rsrpHex);
        // RSRQ
        String rsrqHex = contentHex.substring(122, 126);
        String rsrq = NumberUtils.hexToTenStr(rsrqHex);
        // RSSI
        String rssiHex = contentHex.substring(126, 130);
        short rssi = (short) NumberUtils.hexToTen(rssiHex);
        // SNR
        String snrHex = contentHex.substring(130, 134);
        short snr = (short) NumberUtils.hexToTen(snrHex);
        // 复位次数
        String resetTimesHex = contentHex.substring(134, 142);
        String resetTimes = NumberUtils.hexToTenStr(resetTimesHex);
        // NB模块开启次数
        String openNBTimesHex = contentHex.substring(142, 150);
        String openNBTimes = NumberUtils.hexToTenStr(openNBTimesHex);
        // 后台连接成功测试
        String sbctHex = contentHex.substring(150, 158);
        String sbct = NumberUtils.hexToTenStr(sbctHex);
        // 水表底数
        String waterMeterBaseHex = contentHex.substring(158, 166);
        String waterMeterBase = NumberUtils.hexToTenStr(waterMeterBaseHex);
        // NB上送周期
        String uploadNBCycleHex = contentHex.substring(166, 170);
        String uploadNBCycle = NumberUtils.hexToTenStr(uploadNBCycleHex);
        // 电池电压
        String batteryVoltageHex = contentHex.substring(170, 172);
        String batteryVoltage = NumberUtils.hexToTenStr(batteryVoltageHex);
        // 磁暴标志
        String magneticStormHex = contentHex.substring(172, 174);
        ...
        // RSRP
        combineModel.setRsrp(rsrp);
        // RSRQ
        combineModel.setRsrq(rsrq);
        // RSSI
        ...
        // 水表行数
        combineModel.setWaterConsumption(waterConsumption);
        // 复位次数
        ...
        // 水表底数
        combineModel.setWaterMeterBase(waterMeterBase);
        // NB上送周期
...
//        logger.info(LogMsg.to("resp", resp));
        if (slaveHex.equalsIgnoreCase(totalHex)) {
            ...
            logger.info(LogMsg.to("msg", "升级中了" + slaveHex, "combineModel", combineModel));
        }
        sendMsg(ctx, resp);
    }
