...
import com.dotop.smartwater.project.third.server.meterread.client2.thread.TestRunnable;
...
public class TimerConfig {
    ...
    @Value("${param.config.timer.CheckDeviceValveCommands:10}")
    private int CHECK_DEVICE_VALVE_COMMANDS;

    @Value("${param.config.timer.CheckDeviceValveCommandStatuss:10}")
    private int CHECK_DEVICE_VALVE_COMMAND_STATUSS;

    @Value("${param.config.timer.CheckOwners:5}")
    private int CHECK_OWNERS;

    @Value("${param.config.timer.RefreshDeviceUplinks:5}")
    ...
    public void startCron() {
        Date curr = new Date();
        setCron("TestRunnable", new TestRunnable(), curr, 5, TEST);
